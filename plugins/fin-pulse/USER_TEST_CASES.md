...
验证：插件能把生成的早报通过 OpenAkita 的 IM 网关发送到飞书/企微群，长消息自动按行切分。**IM 通道下拉来自主程序 `SchedulerView` 的 `/api/scheduler/channels`**，显示形式统一为 `channel_id · chat_name`，和主程序计划任务面板完全一致。

### 前置条件

- 0.1 第②步已经配好至少 1 个 IM 通道。
- 用例 3 至少成功生成过一份早报。
- 机器人已经被拉入你测试用的那个群；第一次发消息后，主程序会把 `chat_name` 记住，下拉里会带出群名。
...
| 通道 | 目标 id 长什么样 | 哪里拿 |
|------|-----------------|--------|
| feishu（飞书） | `oc_xxxxxxxxxxxxxxxxxxxx` | 飞书开发者后台 → 机器人 → 添加到群 → 看该群 open_chat_id，或直接让群里 @机器人 一句话，抓回调里的 `chat_id` |
...
1. **设置** 页 → **IM 推送通道** 卡片。卡内先看一遍描述文案确认：
   - 下拉/胶囊里的条目格式是 `feishu · 我的测试群` 这种 `channel · chat_name`。
   - 如果整卡里面出现的是橙色「尚未配置 IM 推送通道」banner（只在本卡内，不会再铺到每个页签顶部），说明你漏了 0.1 第②步。
2. 找到你要测试的通道胶囊（例如 `feishu · 我的测试群`），点旁边的 **「发送测试」** 按钮。
3. 弹出 `chat_id for feishu` 的原生对话框，**默认会把主程序里记录的 chat_id 预填好**；直接确定即可。若为空则粘贴上面拿到的 `chat_id`。
4. 点确定。几秒后：
   - 页面右下角 toast：`sent`（绿色成功条，以前是空白绿条的 bug 已修复）。
...
3. 弹框里的 `channel` 会默认用下拉第一条（来自 `/scheduler/channels`），`chat_id` 默认用它绑定的 chat_id。确定即可。
4. 几秒后群里收到整篇早报；如果 Markdown 超过 25KB，会自动拆成 2–3 条连续消息（由 `finpulse_notification/splitter.py` 做行边界切分，不会把某一条新闻从中间截断）。
...
- **通道胶囊是空的，但你以为配过**：下拉默认从 `/scheduler/channels` 取，只有当主程序里**真正有会话**时才会出现条目。先让机器人在群里说一句话（或接收一条命令），主程序才会记住 `channel + chat_id + chat_name`。如果仍然空，插件会回退到 `/available-channels` 只显示 `channel_id`。
...
验证：插件里可以通过 **业务模板按钮**（早报/午报/晚报/雷达）创建计划任务，创建完**底层数据直接落在主程序 `TaskScheduler` 里**，到点自动生成早报并推到 IM。

> **架构说明（必读）**：V1.0 起，插件只保留「业务模板 + 创建表单」这段最贴近财经场景的 UI；任务的**列表管理/暂停/删除**全部交给 OpenAkita 主程序的 **计划任务面板** 统一托管。插件 Settings 里的任务列表现在是 **只读预览**，右上角有一个 **「在主程序计划任务面板管理 →」** 的跳转按钮。

### 前置条件

- 用例 4A 已经把 IM 通道下拉测通（至少有 1 条带 `chat_name` 的条目）。
- OpenAkita 主程序保持常驻运行（APScheduler 不跨进程唤醒）。

### 素材从哪里来

仍然是用例 4 里拿到的 `chat_id`。下拉里挑一个带 `chat_name` 的条目即可。

### 操作步骤

1. **设置** 页 → **定时任务** 卡片。
2. 卡片上方有一行说明和一行 **4 个模板按钮**（自适应网格）：
   - `每日早报` — 默认 cron `0 8 * * *`、session=morning、回看 12h、top_k=20
   - `每日午报` — 默认 cron `30 12 * * *` / noon / 12h / 15
   - `每日晚报` — 默认 cron `0 19 * * *` / evening / 12h / 20
   - `雷达预警` — 默认 cron `*/15 * * * *` / 读取当前 `config.radar_rules` 预填规则
3. 点 **「每日早报」**，弹出命名对话框（标题形如 `创建定时任务 — 每日早报`），里面是结构化表单：
   - **Cron 表达式**：默认 `0 8 * * *`，下方有 [crontab.guru](https://crontab.guru) 链接 + 本地时区显示。
   - **IM 通道**：下拉（来自 `/scheduler/channels`，条目形如 `feishu · 我的测试群`）。选中时自动把它绑的 `chat_id` 预填。
   - **chat_id**：默认取所选通道的 chat_id，可手动覆盖。
   - 雷达模板额外有一个多行的 **关键词规则** 文本框，默认预填当前 `radar_rules`。
4. 点 **「创建」**。成功后：
   - 页面右下角 toast：`scheduled <task_id>`。
   - 卡片下面只读列表里多一行：`fin-pulse morning` · `0 8 * * *` · `feishu · oc_xxx`。

### 去主程序面板确认

1. 点卡片右上角 **「在主程序计划任务面板管理 →」**。
2. 主程序跳到 `计划任务` 视图，能看到一条名字以 `fin-pulse ` 开头（注意是 **空格** 分隔，不是冒号）的任务。
3. 在那里可以：
   - 查看下次触发时间、手动触发一次、查看执行历史。
   - 暂停/恢复/删除（**这些操作不再暴露在插件 UI 里**）。

### 常用 cron 模板（复制即用）

| 目的 | cron 值 | 备注 |
|------|---------|------|
| 每天 8:00 早报 | `0 8 * * *` | 早报模板默认值 |
| 每天 12:30 午报 | `30 12 * * *` | 午报模板默认值 |
| 每天 19:00 晚报 | `0 19 * * *` | 晚报模板默认值 |
| 工作日每 15 分钟雷达 | `*/15 * * * 1-5` | 雷达模板，加日限定 |
| 每小时整点雷达 | `0 * * * *` | 雷达模板，简化版 |

### 预期看到

- 到了 `0 8 * * *`（快速验证可以把 cron 改成"当前时间 + 2 分钟"），群里会收到一份新鲜早报。
- 同时 **早午晚报** 页签里也会多出一张同一时间戳的卡片。
- 主程序计划任务面板的「最近执行」列表里也会多出一条 fin-pulse 记录。

### 不对劲

- **弹窗里 IM 通道下拉是空的**：回到用例 4，让机器人在目标群说一句话，主程序才会记住 `chat_name`。也可以先直接手填 `channel_id` + `chat_id` 发一次测试，让主程序补上记录。
- **到点没触发**：OpenAkita 主程序必须开着。桌面/服务器休眠时 APScheduler 不会补跑；主程序计划任务面板 → 任务详情可以看 `last_run` / `next_run` 判断。
- **cron 语法错误**：前端没有严格校验，错误的表达式会在主程序日志里抛 `CronTrigger` 异常。推荐用 [crontab.guru](https://crontab.guru) 先验证表达式。
- **想删除老任务**：不要在插件里找删除按钮（已移除），去主程序计划任务面板删；插件会自动兼容 `fin-pulse:` 老前缀和 `fin-pulse ` 新前缀两种命名。
